...
/* PROFILE UUID */
#define MIBAND_PF_UUID_USER            0xff04
#define MIBAND_PF_UUID_CONTROL         0xff05
#define MIBAND_PF_UUID_REALTIME        0xff06
#define MIBAND_PF_UUID_LEPARAM         0xff09
#define MIBAND_PF_UUID_BATTERY         0xff0c
#define MIBAND_PF_UUID_TEST            0xff0d
#define MIBAND_PF_UUID_SENSOR          0xff0e
#define MIBAND_PF_UUID_PAIR            0xff0f
#define MIBAND_PF_UUID_VIBRATION       0x2a06

/* PROFILE Handle */
#define MIBAND_PF_HND_USER             0x0019
#define MIBAND_PF_HND_CONTROL          0x001b
#define MIBAND_PF_HND_REALTIME         0x001d
#define MIBAND_PF_HND_REALTIME_NOTIFY  0x001e
#define MIBAND_PF_HND_LEPARAM          0x0025
#define MIBAND_PF_HND_LEPARAM_NOTIFY   0x0026
#define MIBAND_PF_HND_BATTERY          0x002c
#define MIBAND_PF_HND_BATTERY_NOTIFY   0x002d
#define MIBAND_PF_HND_TEST             0x002f
#define MIBAND_PF_HND_SENSOR           0x0031
#define MIBAND_PF_HND_SENSOR_NOTIFY    0x0032
#define MIBAND_PF_HND_PAIR             0x0034
#define MIBAND_PF_HND_VIBRATION        0x0051
...
int mmb_miband_op_error(MMB_MIBAND * UNUSED(this), uint16_t hnd, uint8_t error_code)
{
    printf("[MMB][MIBAND][OP] ERROR - Hnd = 0x%04x, ErrCode = %d.\n", hnd, error_code);
    return 0;
}
...
int mmb_miband_op_read_type_resp(MMB_MIBAND * this, uint16_t hnd, uint8_t *val, size_t size)
{
    ...
    switch (hnd)
    {
        case MIBAND_PF_HND_BATTERY:
            ret = mmb_miband_update_battery_data(this, val, size);
            break;

        default:
            printf("[MMB][MIBAND][OP][ReadTypeResp] Unknow Hnd = 0x%04x.\n", hnd);
            ret = -1;
            break;
    }
...
int mmb_miband_op_read_resp(MMB_MIBAND * UNUSED(this), uint8_t *UNUSED(val), size_t UNUSED(size))
{
    return 0;
}
...
int mmb_miband_op_notification(MMB_MIBAND * this, uint16_t hnd, uint8_t *val, size_t size)
{
    ...
    switch (hnd)
    {
        ...
        case MIBAND_PF_HND_SENSOR:
            ret = mmb_miband_update_sensor_data(this, val, size);
            break;

        default:
            printf("[MMB][MIBAND][OP][NOTIFICATION] unknow hnd[0x%04x] size[%ld] ", hnd, size);
